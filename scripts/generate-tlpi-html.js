...
/**
 * TLPI HTML Generator
 * Generates standalone HTML pages with Monaco Editor for each source file
 */
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .stat-value{font-size:1.5rem;font-weight:700;color:#4ec9b0}
    .stat-label{font-size:0.875rem;color:#808080}
    .category{margin-bottom:32px}
    .category-title{color:#569cd6;font-size:1.25rem;margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid #3c3c3c}
...
function generateIndex(files) {
  ...
  stats += '<div class="stat"><div class="stat-value">' + files.length + '</div><div class="stat-label">Total Files</div></div>';
  stats += '<div class="stat"><div class="stat-value">' + cFiles + '</div><div class="stat-label">C Files</div></div>';
  stats += '<div class="stat"><div class="stat-value">' + hFiles + '</div><div class="stat-label">Headers</div></div>';
  stats += '<div class="stat"><div class="stat-value">' + shFiles + '</div><div class="stat-label">Shell Scripts</div></div>';
  stats += '<div class="stat"><div class="stat-value">' + categories.length + '</div><div class="stat-label">Categories</div></div>';
  stats += '</div>';

  let content = '';
  for (const cat of categories) {
    const catFiles = grouped[cat].sort((a, b) => a.filename.localeCompare(b.filename));
    content += '<div class="category"><h2 class="category-title">' + cat + '</h2><div class="file-grid">';
    for (const f of catFiles) {
      const iconBg = FILE_TYPES[f.ext] ? FILE_TYPES[f.ext].color : '#555';
      const iconText = f.ext === '.h' ? 'H' : f.ext === '.c' ? 'C' : 'SH';
      content += '<a href="' + f.category + '/' + f.filename + '.html" class="file-card">';
      content += '<div class="file-icon" style="background:' + iconBg + '">' + iconText + '</div>';
      content += '<div class="file-name">' + f.filename + '</div></a>';
    }
    content += '</div></div>';
  }

  return getIndexTemplate().replace('{{STATS}}', stats).replace('{{CONTENT}}', content);
}
...
function findFiles(dir, baseDir) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        files.push.apply(files, findFiles(fullPath, baseDir));
      } else {
...
function main() {
  console.log('TLPI HTML Generator');
  console.log('===================\n');
  ...
  for (const file of files) {
    try {
      ...
      const html = generateHtml(file.filename, code, file.category, config);
      const outDir = path.join(OUTPUT_DIR, file.category);
      if (!fs.existsSync(outDir)) {
        fs.mkdirSync(outDir, { recursive: true });
      }
      fs.writeFileSync(path.join(outDir, file.filename + '.html'), html);
      generated++;
      console.log('Generated:', file.category + '/' + file.filename + '.html');
    } catch (err) {
  ...
  console.log('\nGenerating index...');
  fs.writeFileSync(path.join(OUTPUT_DIR, 'index.html'), generateIndex(files));
